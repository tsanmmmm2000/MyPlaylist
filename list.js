...
window.addEventListener('DOMContentLoaded', function () {
	...
	chrome.tabs.query({
		...
		chrome.tabs.sendMessage(tabs[0].id, { from: 'popup', subject: 'DOMInfo' }, WriteList);
	});
});
*/
...
function SetSortingList() {
	...
	$.each($("li"), function(i,n) {
		var key = $(this).children("div")[0].id;
		sortingList[i] = key;
	});
	chrome.storage.local.remove('SortingList');
	chrome.storage.local.set({'SortingList': sortingList});
}

function SortList(playlist) {
	var newList = playlist;
	chrome.storage.local.get('SortingList', function(result) {
		var sortingList = result.SortingList;
		if (sortingList != null && sortingList != undefined && !$.isEmptyObject(sortingList)) {
			newList = {};
			$.each(sortingList, function(i,n){
				var listItem = playlist[n];
